...
import { 
  User, Lock, Upload, FileText, CheckCircle, XCircle, 
  Calendar, Search, LogOut, ChevronDown, ChevronUp, 
  Trash2, Eye, Download, ZoomIn, ZoomOut, RefreshCw,
  Folder, FolderOpen, Mail, Clock, AlertTriangle, Settings, Plus,
...
import { 
  ...
  doc, onSnapshot, query, where, orderBy, getDocs, setDoc
    ...
    <div className="fixed inset-0 z-[80] bg-black bg-opacity-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white rounded-lg shadow-xl max-w-sm w-full p-6 animate-scale-in">
        ...
        <div className="flex justify-end gap-3">
          <button onClick={onCancel} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg transition font-medium">取消</button>
          <button onClick={onConfirm} className="px-4 py-2 bg-indigo-600 text-white hover:bg-indigo-700 rounded-lg shadow transition font-medium">確認</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
          ...
          if (u.email && u.email.toLowerCase() === ADMIN_EMAIL.toLowerCase()) {
              setCurrentProfile({ role: 'admin', name: '管理員', id: u.uid, email: u.email });
              setView('admin');
              return; // 結束，不繼續執行學生邏輯
          }
  ...
  const handleAdminLogin = async (credentials) => {
      try {
        ...
        // useEffect 裡面的邏輯會判斷 email === ADMIN_EMAIL 然後自動轉導
          ...
          if (error.code === 'auth/operation-not-allowed') msg = "請至 Firebase Console 開啟 Email/Password 登入功能";
          showNotification(msg, 'error');
      }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
      {notification && (
        <div className={`fixed top-4 right-4 z-[90] px-6 py-3 rounded shadow-lg text-white ${
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-indigo-100 to-blue-50 p-4">
      {showAnnouncement && (
        <div className="absolute top-0 left-0 w-full bg-yellow-100 border-b border-yellow-200 p-3 shadow-sm z-10 animate-fade-in-down">
          <div className="max-w-md mx-auto flex items-start gap-3">
             <Megaphone size={20} className="text-yellow-600 flex-shrink-0 mt-0.5" />
             <div className="flex-1">
               <p className="text-yellow-800 text-sm font-medium whitespace-pre-wrap">{announcement.content}</p>
             </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-indigo-50 mt-12">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-indigo-900 mb-2">陳蒂國文補習班</h1>
          <p className="text-gray-500">雲端作文繳交系統</p>
        </div>

        <div className="flex mb-6 bg-gray-100 p-1 rounded-lg">
          <button 
            className={`flex-1 py-2 rounded-md text-sm font-medium transition-all ${role === 'student' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            onClick={() => setRole('student')}
          >
            學員登入/註冊
          </button>
          <button 
            className={`flex-1 py-2 rounded-md text-sm font-medium transition-all ${role === 'admin' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
      ...
      <div className="p-6 space-y-4">
        <div className="space-y-3 text-gray-700">
          <div className="flex items-start gap-3">
            ...
            <p><strong>用黑原子筆</strong>：字跡清晰，勿用鉛筆。</p>
          </div>
          <div className="flex items-start gap-3">
            <CheckCircle className="text-green-500 flex-shrink-0 mt-0.5" size={20} />
            <p><strong>標示篇名</strong>：第一行空四格寫題目。</p>
          </div>
          <div className="flex items-start gap-3">
            <CheckCircle className="text-green-500 flex-shrink-0 mt-0.5" size={20} />
            <p><strong>光線充足</strong>：在亮處拍攝，無陰影。</p>
          </div>
          <div className="flex items-start gap-3">
            <CheckCircle className="text-green-500 flex-shrink-0 mt-0.5" size={20} />
            <p><strong>水平拍攝</strong>：鏡頭對準正上方，不歪斜。</p>
          </div>
        </div>
...
const ImageLightbox = ({ src, onClose }) => (
    <div className="fixed inset-0 z-[70] bg-black bg-opacity-90 flex items-center justify-center cursor-pointer" onClick={onClose}>
        <img src={src} className="max-w-[90vw] max-h-[90vh] object-contain shadow-2xl" alt="Full size guide" />
    </div>
);
  ...
  return (
    <div className="w-full h-32 mt-4 bg-indigo-50 rounded-lg p-4 relative border border-indigo-100">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-20 overflow-visible" preserveAspectRatio="none">
        ...
        <line x1="0" y1="50" x2="100" y2="50" stroke="#e0e7ff" strokeWidth="1" />
        <line x1="0" y1="100" x2="100" y2="100" stroke="#e0e7ff" strokeWidth="1" />
        ...
        {scores.map((score, index) => {
           ...
           const y = height - (score / maxScore) * height;
           return (
             <circle key={index} cx={x} cy={y} r="3" fill="#4f46e5" vectorEffect="non-scaling-stroke" />
           );
        })}
      </svg>
    </div>
...
const SubmissionViewerModal = ({ submission, onClose }) => {
    return (
        <div className="fixed inset-0 z-[70] bg-black bg-opacity-90 flex items-center justify-center p-4" onClick={onClose}>
            <div className="relative max-w-4xl w-full bg-white rounded-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                <div className="p-4 bg-gray-50 border-b flex justify-between items-center">
                    <h3 className="font-bold">繳交檢視: {formatDateTime(submission.date)}</h3>
                ...
                <div className="flex-1 overflow-auto p-4 bg-gray-100 flex justify-center">
                    <div className="space-y-4 w-full max-w-2xl">
                         {submission.imageUrls && submission.imageUrls.map((url, idx) => (
                             <img key={idx} src={url} alt={`page-${idx}`} className="w-full h-auto rounded shadow-sm border" />
                         ))}
                         {!submission.imageUrls && submission.imageUrl && (
                             <img src={submission.imageUrl} alt="submission" className="w-full h-auto rounded shadow-sm border" />
                         )}
                    </div>
                </div>
                {submission.status === 'graded' && (
                    <div className="p-4 bg-white border-t">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-xl font-bold text-indigo-600">{submission.score}分 <span className="text-sm text-gray-500">/ {getMaxScore(submission.topic)}分</span></span>
                        </div>
                        <p className="text-gray-600 text-sm whitespace-pre-wrap">{submission.comment}</p>
                    </div>
                )}
            </div>
        </div>
...
function StudentDashboard({ user, submissions, announcement, onLogout, showNotification }) {
  ...
  const scoreTrend = useMemo(() => 
    [...gradedSubmissions].sort((a, b) => new Date(a.date) - new Date(b.date)).map(s => s.score),
  [gradedSubmissions]);

  const remainingDays = calculateRemainingDays(user.expiryDate);
  const isExpiredAccount = remainingDays <= 0;
  
  const submittedToday = mySubmissions.some(s => isSameDay(s.date, new Date()));
  const countSubmitted = mySubmissions.length;
  const isLimitReached = countSubmitted >= COURSE_SETTINGS.maxEssays;

  const canSubmit = !isExpiredAccount && !user.isManualLocked && !submittedToday && !isLimitReached;
  ...
  const showAnnouncement = useMemo(() => {
    if (!announcement || !announcement.visible || !announcement.content) return false;
    if (!announcement.targetTags || announcement.targetTags.length === 0) return true;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (let i = 0; i < compressedImages.length; i++) {
          const imgBase64 = compressedImages[i];
          const fileName = `${Date.now()}_${i}.jpg`;
          const storageRef = ref(storage, `submissions/${user.id}/${fileName}`);
      ...
      const newSubmission = {
        ...
        imageUrls: uploadedUrls, // 存入 URL
        imageUrl: uploadedUrls[0], // 相容舊欄位
        storagePath: `submissions/${user.id}`, 
        status: 'pending',
        score: null,
        comment: '',
        topic: selectedTopic
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-8">
      {showLoginNotice && <LoginNoticeModal onClose={() => setShowLoginNotice(false)} />}
      ...
      <header className="flex justify-between items-center mb-8 bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">歡迎回來，{user.name} 同學</h1>
          <p className="text-gray-500 mt-1">課程期限: {formatDate(user.expiryDate)}</p>
          <div className="flex gap-2 mt-2">
            {user.tags && user.tags.map((tag, idx) => (
                <span key={idx} className="bg-indigo-50 text-indigo-600 text-xs px-2 py-0.5 rounded-full font-medium">#{tag}</span>
            ))}
          </div>
        </div>
        <div className="flex gap-3 flex-wrap">
          <button onClick={onLogout} className="flex items-center gap-2 text-gray-600 hover:text-red-500 px-4 py-2 rounded-lg hover:bg-red-50 transition border border-gray-200 bg-white">
            <LogOut size={18} /> 登出
          </button>
        </div>
      </header>
      ...
      {showAnnouncement && (
        <div className="mb-8 bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded shadow-sm flex items-start gap-4 animate-fade-in-down">
          ...
          <div>
            ...
            <p className="text-yellow-700 mt-1 whitespace-pre-wrap">{String(announcement.content)}</p>
            <p className="text-yellow-600 text-xs mt-2 text-right">發布時間: {formatDateTime(announcement.updatedAt)}</p>
          </div>
        </div>
      )}
      ...
      {gradedSubmissions.length > 0 && (
        <div className="mb-8 bg-green-50 border border-green-200 rounded-xl p-4 flex items-start gap-4 shadow-sm">
          ...
          <div className="flex-1">
            <h3 className="font-bold text-green-800 text-lg">您有 {gradedSubmissions.length} 篇作文已完成批改！</h3>
            <p className="text-green-700 text-sm mt-1">
              最新的批改：{formatDate(gradedSubmissions[0].date)} 的作業，
              得分 <span className="font-bold text-lg">{gradedSubmissions[0].score}</span> 分。
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className={`bg-gradient-to-br ${remainingDaysCardStyle} rounded-xl p-6 text-white shadow-lg transition-colors duration-500`}>
          <div className="flex items-center gap-3 mb-2">
            <Clock className="opacity-80" />
            <h3 className="font-semibold text-lg">剩餘課程天數</h3>
          </div>
          <p className="text-4xl font-bold">{remainingDays} <span className="text-lg font-normal opacity-80">天</span></p>
          {isExpiredAccount && <p className="mt-2 bg-white/20 inline-block px-2 py-1 rounded text-xs">課程已到期</p>}
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          <p className="text-4xl font-bold text-gray-800">{countSubmitted} <span className="text-lg font-normal text-gray-400">/ {COURSE_SETTINGS.maxEssays}</span></p>
          <div className="w-full bg-gray-200 rounded-full h-2.5 mt-3">
            <div className="bg-indigo-600 h-2.5 rounded-full" style={{ width: `${Math.min(100, (countSubmitted / COURSE_SETTINGS.maxEssays) * 100)}%` }}></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          {user.isManualLocked && <p className="text-red-500 text-sm mt-1">已被老師暫停繳交權限</p>}
        </div>
      </div>
            ...
            <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2 flex items-center gap-1">
                        <BookOpen size={16} className="text-indigo-600"/> 選擇作文主題
                    </label>
                    <select
                      value={selectedTopic}
                      ...
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                    >
                      {ESSAY_TOPICS.map(t => <option key={t} value={t}>{t}</option>)}
                    </select>
                  </div>

                  <label 
                    className={`block w-full cursor-pointer group relative ${isDragging ? 'scale-105' : ''} transition-transform`}
                    onDragOver={handleDragOver}
                    onDragLeave={handleDragLeave}
                    onDrop={handleDrop}
                  >
                    <input type="file" className="hidden" accept="image/*" multiple onChange={handleFileChange} />
                    <div className={`w-full h-48 border-2 border-dashed ${isDragging ? 'border-indigo-500 bg-indigo-50' : 'border-indigo-200 bg-indigo-50'} rounded-xl flex flex-col items-center justify-center group-hover:bg-indigo-100 transition text-indigo-400`}>
                      <Upload size={32} className={`mb-2 ${isDragging ? 'animate-bounce' : ''}`} />
                      <span className="text-sm font-medium">
                          {files.length > 0 ? `已選擇 ${files.length} 張圖片` : "點擊 / 拖曳圖片 (最多3張)"}
                      </span>
                    </div>
                  </label>
                  ...
                  <button 
                    ...
                    className={`w-full py-2.5 rounded-lg font-semibold transition flex items-center justify-center gap-2 ${files.length > 0 && !isUploading ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
                  >
                    {isUploading ? <Loader className="animate-spin" size={20} /> : <CheckCircle size={20} />}
              ...
              <div className="mt-6 pt-6 border-t border-gray-100">
                <h4 className="text-sm font-bold text-gray-700 flex items-center gap-2 mb-3">
                   <Info size={16} className="text-indigo-500"/> 上傳注意事項 (點圖可放大)
                </h4>
                ...
                <div 
                    ...
                    onClick={() => setLightboxImage('https://www.chenti-chinese.com/wp-content/uploads/2025/12/Gemini_Generated_Image_h9s8vxh9s8vxh9s8-e1766481893338.png')}
                >
                  <img 
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 min-h-[500px]">
            ...
            <div className="divide-y divide-gray-100">
              {mySubmissions.length === 0 ? (
                <div className="p-8 text-center text-gray-400">尚無繳交紀錄</div>
              ) : (
                mySubmissions.map(sub => (
                  <div key={sub.id} className="p-4 hover:bg-gray-50 transition flex flex-col sm:flex-row gap-4">
                    <div className="w-full sm:w-32 h-32 bg-gray-200 rounded-lg overflow-hidden flex-shrink-0 border border-gray-200 cursor-zoom-in relative group" onClick={() => setViewSubmission(sub)}>
                      {/* 顯示第一張圖 */}
                      <img src={sub.imageUrls ? sub.imageUrls[0] : sub.imageUrl} alt="essay" className="w-full h-full object-cover" />
                      ...
                      {sub.status === 'graded' && (
                        <div className="mt-3 bg-green-50 p-3 rounded-lg border border-green-100">
                          <div className="flex justify-between items-center mb-1">
                            <span className="font-bold text-green-800">老師評語：</span>
                            <span className="font-bold text-xl text-indigo-600">{sub.score} <span className="text-sm text-gray-500">/ {getMaxScore(sub.topic)}分</span></span>
                          </div>
                          <p className="text-gray-700 text-sm whitespace-pre-wrap">{sub.comment}</p>
                        </div>
                      )}
                    </div>
                  </div>
...
function AdminDashboard({ users, submissions, announcement, onLogout, showNotification }) {
  const [activeTab, setActiveTab] = useState('folders'); 
  const [selectedMonth, setSelectedMonth] = useState(null);
  const [sortConfig, setSortConfig] = useState({ key: 'remainingDays', direction: 'asc' });
  ...
  const [announcementText, setAnnouncementText] = useState(announcement?.content || ''); 
  const [announcementTargetTags, setAnnouncementTargetTags] = useState(announcement?.targetTags || []);
  const [confirmConfig, setConfirmConfig] = useState({ isOpen: false, message: '', onConfirm: null });
  ...
  const riskAnalysis = useMemo(() => {
    ...
    users.forEach(u => {
      ...
      const lastSubDate = userSubs.length > 0 
        ? new Date(Math.max(...userSubs.map(s => new Date(s.date)))) 
        : new Date(u.startDate);
      
      const lastLoginDate = u.lastLoginAt ? new Date(u.lastLoginAt) : new Date(u.startDate);
      const lastActivity = new Date(Math.max(lastSubDate.getTime(), lastLoginDate.getTime()));
      ...
      if (gradedSubs.length >= 3) {
        if (gradedSubs[0].score < gradedSubs[1].score && gradedSubs[1].score < gradedSubs[2].score) {
          regressionRisks.push({ 
            user: u, 
            scores: [gradedSubs[2].score, gradedSubs[1].score, gradedSubs[0].score]
          });
        }
      }
    });
  ...
  const totalRisks = riskAnalysis.regressionRisks.length + riskAnalysis.inactivityRisks.length;

  const dbUsage = useMemo(() => estimateDbUsage(users, submissions), [users, submissions]);
  const usagePercentage = Math.min(100, (dbUsage / COURSE_SETTINGS.dbSizeLimitBytes) * 100);
  ...
  const monthAnalytics = useMemo(() => {
    ...
    const allSubsInMonth = submissionsByMonth.find(g => g[0] === selectedMonth)?.[1] || [];
    const gradedSubs = allSubsInMonth.filter(s => s.status === 'graded' && typeof s.score === 'number');
    ...
    ESSAY_TOPICS.forEach(topic => {
      ...
      if (topicSubs.length > 0) {
        const topicAvg = (topicSubs.reduce((a, b) => a + b.score, 0) / topicSubs.length).toFixed(1);
  ...
  const currentViewSubmissions = useMemo(() => {
    if (activeTab === 'folders' && selectedMonth) {
      let subs = submissionsByMonth.find(g => g[0] === selectedMonth)?.[1] || [];
    ...
    return [];
  }, [activeTab, selectedMonth, submissionsByMonth, filterStatus, filterTag, users]);
  ...
  const sortedUsers = useMemo(() => {
    ...
    if (searchTerm) {
      sortableUsers = sortableUsers.filter(u => 
        (u.name && u.name.includes(searchTerm)) || (u.phone && u.phone.includes(searchTerm))
      );
    }

    if (filterTag !== 'all') {
        sortableUsers = sortableUsers.filter(u => u.tags && u.tags.includes(filterTag));
    }
  ...
  const handleSaveUserEdit = () => {
      ...
      // 檢查姓名唯一性 (排除自己)
      const duplicateNameUser = users.find(u => u.name === trimmedName && u.id !== editingUser.id);
      if (duplicateNameUser) {
          showNotification(`無法修改：姓名 "${trimmedName}" 已被手機 ${duplicateNameUser.phone} 註冊`, 'error');
          return;
      }

      // 檢查手機唯一性 (排除自己)
      const duplicatePhoneUser = users.find(u => u.phone === trimmedPhone && u.id !== editingUser.id);
      if (duplicatePhoneUser) {
          showNotification(`無法修改：手機 "${trimmedPhone}" 已被學員 ${duplicatePhoneUser.name} 註冊`, 'error');
          return;
      }
  ...
  const navigateSubmission = (direction) => {
    if (!gradingItem) return;
    const currentIndex = currentViewSubmissions.findIndex(s => s.id === gradingItem.id);
    ...
    if (newIndex >= 0 && newIndex < currentViewSubmissions.length) {
      ...
      showNotification(direction === 'next' ? '已經是最後一篇了' : '已經是第一篇了', 'info');
    }
  };

  const saveGrading = async (shouldNavigateNext = false) => {
    const currentMax = getMaxScore(gradingItem.topic);
    if (gradingData.score === '' || isNaN(gradingData.score) || gradingData.score < 0 || gradingData.score > currentMax) {
      showNotification(`請輸入 0-${currentMax} 之間的有效分數`, 'error');
      return;
    }

    try {
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'submissions', gradingItem.id), {
        status: 'graded',
  ...
  const handleSaveAnnouncement = async (visible) => {
    try {
        await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'system_announcement', 'main'), {
  ...
  const exportStudentData = async (user) => {
    ...
    try {
      ...
      if (!window.JSZip || !window.saveAs) throw new Error('Zip Lib Load Failed');
      ...
      const promises = userSubs.map(async (sub, idx) => {
        ...
        textReport += `狀態: ${sub.status === 'graded' ? '已批改' : '未批改'}\n`;
  ...
  const generatePortfolio = async (user) => {
    ...
    try {
      ...
      const userSubs = submissions.filter(s => s.studentId === user.id && s.status === 'graded');
      if (userSubs.length === 0) {
      ...
      const avgScore = (userSubs.reduce((acc, curr) => acc + curr.score, 0) / userSubs.length).toFixed(1);
      ...
      Object.assign(page1.style, {
        width: "210mm", minHeight: "297mm", backgroundColor: "white", padding: "20mm", fontFamily: "sans-serif"
      });
      ...
      page1.innerHTML = `
        ...
        <div style="background: #f8fafc; padding: 20px; border-radius: 10px; border: 1px solid #e2e8f0; margin-bottom: 30px;">
          ...
          <p><strong>統計期間：</strong>${formatDate(user.startDate)} ~ ${formatDate(user.expiryDate)}</p>
        </div>
        ...
        <div style="display: flex; gap: 20px; margin-bottom: 30px;">
           <div style="flex: 1; background: #ecfccb; padding: 20px; border-radius: 10px; text-align: center;">
              ...
              <div style="font-size: 36px; font-weight: bold; color: #365314;">${userSubs.length}</div>
           </div>
           ...
           <div style="flex: 1; background: #ffedd5; padding: 20px; border-radius: 10px; text-align: center;">
              ...
              <div style="font-size: 36px; font-weight: bold; color: #7c2d12;">${Math.max(...userSubs.map(s => s.score))}</div>
           </div>
        </div>
      `;
      ...
      for (let i = 0; i < top3.length; i++) {
        ...
        pageEssay.innerHTML = `
           <div style="border-bottom: 2px solid #f59e0b; padding-bottom: 15px; margin-bottom: 30px;">
             ...
             <p style="margin: 5px 0 0; color: #666;">繳交日期：${formatDateTime(sub.date)}</p>
           </div>
           ...
           <div style="display: flex; justify-content: space-between; align-items: center; background: #fffbeb; padding: 20px; border-radius: 10px; margin-bottom: 30px;">
              <div>
                 ...
                 <div style="font-size: 48px; font-weight: bold; color: #b45309;">${sub.score}</div>
              </div>
              <div style="flex: 1; margin-left: 30px;">
                 <h4 style="margin: 0 0 10px; color: #92400e;">老師評語：</h4>
                 <p style="margin: 0; white-space: pre-wrap; color: #333; line-height: 1.6;">${sub.comment || "無評語"}</p>
              </div>
           </div>

           <div style="text-align: center; border: 1px solid #ddd; padding: 10px; border-radius: 5px;">
              <img src="${sub.imageUrl}" style="max-width: 100%; max-height: 140mm; display: block; margin: 0 auto;" />
           </div>
        `;
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <nav className="bg-indigo-900 text-white shadow-lg sticky top-0 z-30">
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <div className="hidden md:flex space-x-2">
                <button 
                  onClick={() => { setActiveTab('folders'); setSelectedMonth(null); setFilterStatus('all'); setFilterTag('all'); }}
                  className={`px-3 py-2 rounded-md text-sm font-medium ${activeTab === 'folders' ? 'bg-indigo-700' : 'hover:bg-indigo-800'}`}
                >
                  作文批改
                </button>
                <button 
                  onClick={() => { setActiveTab('students'); setFilterTag('all'); }}
                  className={`px-3 py-2 rounded-md text-sm font-medium ${activeTab === 'students' ? 'bg-indigo-700' : 'hover:bg-indigo-800'}`}
                >
                  學員列表
                </button>
                <button 
                  onClick={() => setActiveTab('alerts')}
                  className={`px-3 py-2 rounded-md text-sm font-medium flex items-center gap-1 ${activeTab === 'alerts' ? 'bg-red-700' : 'hover:bg-indigo-800'}`}
                >
                  <Siren size={16} className={totalRisks > 0 ? "animate-pulse text-red-300" : ""} /> 
                  預警中心
                  {totalRisks > 0 && <span className="bg-red-500 text-white text-xs px-1.5 rounded-full ml-1">{totalRisks}</span>}
                </button>
              </div>
            </div>
            <div className="flex items-center gap-4">
                ...
                {/* 警示橫幅 */}
                {usagePercentage > 80 && (
                    <div className={`hidden md:flex items-center gap-1 text-xs px-2 py-1 rounded font-bold ${usagePercentage > 90 ? 'bg-red-600 text-white animate-pulse' : 'bg-yellow-500 text-white'}`} title="資料庫容量警示">
                        <Database size={14}/> {usagePercentage.toFixed(0)}%
                    </div>
                )}
                <button onClick={onLogout} className="text-gray-300 hover:text-white px-3 py-2 rounded-md text-sm font-medium">
                登出
                </button>
            </div>
          </div>
        ...
        <div className="bg-indigo-800 h-1.5 w-full relative group cursor-help">
            <div 
                className={`h-full transition-all duration-500 ${usagePercentage > 90 ? 'bg-red-500' : usagePercentage > 70 ? 'bg-yellow-400' : 'bg-green-400'}`} 
            ...
            <div className="absolute top-2 left-0 bg-black text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50">
                資料庫使用量: {formatBytes(dbUsage)} / {formatBytes(COURSE_SETTINGS.dbSizeLimitBytes)} ({usagePercentage.toFixed(2)}%)
            </div>
        </div>
      </nav>
        ...
        {activeTab === 'alerts' && (
           <div className="space-y-6">
             <div className="bg-white rounded-lg shadow p-6 border-l-4 border-red-500">
               ...
               <p className="text-gray-600 mt-1">系統自動偵測需關注的學員，請行政人員主動聯繫關懷。</p>
             </div>
             ...
             <div className="bg-white rounded-lg shadow p-6 border-l-4 border-indigo-500">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-bold text-gray-600">
                    {formatBytes(dbUsage)} / {formatBytes(COURSE_SETTINGS.dbSizeLimitBytes)} ({usagePercentage.toFixed(2)}%)
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                  <div 
                    className={`h-full transition-all duration-500 ${
                      usagePercentage > 90 ? 'bg-red-600 animate-pulse' : 
                ...
                <p className="text-xs text-gray-500 mt-2 text-right">
                  {usagePercentage > 90 ? '⚠️ 容量即將額滿，請盡快備份並刪除舊生資料！' : 
                   usagePercentage > 70 ? '⚠️ 容量使用率較高，建議開始規劃清理資料。' : 
                   '✅ 目前容量充足。'}
                </p>
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="bg-white rounded-lg shadow overflow-hidden">
                 <div className="p-4 bg-orange-50 border-b border-orange-100 flex justify-between items-center">
                   ...
                   <span className="bg-orange-200 text-orange-800 text-xs px-2 py-1 rounded-full">{riskAnalysis.regressionRisks.length} 人</span>
                 </div>
                 <div className="divide-y divide-gray-100 max-h-[500px] overflow-y-auto">
                   {riskAnalysis.regressionRisks.length === 0 ? (
                     <div className="p-8 text-center text-gray-400">目前沒有連續退步的學員，太棒了！</div>
                   ) : (
                     riskAnalysis.regressionRisks.map(({ user, scores }) => (
                       <div key={user.id} className="p-4 hover:bg-gray-50 flex justify-between items-center">
                         ...
                         <div className="text-right">
                           <div className="text-xs text-gray-500 mb-1">近期分數走勢</div>
                           <div className="flex items-center gap-1 font-mono font-bold text-red-600">
                             {scores.map((s, i) => (
                               <React.Fragment key={i}>
                                 <span>{s}</span>
                                 {i < scores.length - 1 && <span className="text-gray-300">→</span>}
                               </React.Fragment>
                             ))}
                           </div>
                         </div>
                         <button onClick={() => setEditingUser(user)} className="p-2 text-gray-400 hover:text-indigo-600">
                           <Settings size={18} />
                         </button>
                       </div>
                     ))
               ...
               <div className="bg-white rounded-lg shadow overflow-hidden">
                 <div className="p-4 bg-red-50 border-b border-red-100 flex justify-between items-center">
                   ...
                   <span className="bg-red-200 text-red-800 text-xs px-2 py-1 rounded-full">{riskAnalysis.inactivityRisks.length} 人</span>
                 </div>
                 <div className="divide-y divide-gray-100 max-h-[500px] overflow-y-auto">
                    {riskAnalysis.inactivityRisks.length === 0 ? (
                     <div className="p-8 text-center text-gray-400">所有學員近期皆有活動。</div>
                   ) : (
                     riskAnalysis.inactivityRisks.map(({ user, days, lastActive }) => (
                       <div key={user.id} className="p-4 hover:bg-gray-50 flex justify-between items-center">
                         <div>
                           <div className="font-bold text-gray-800">{user.name}</div>
                           <div className="text-xs text-gray-500">{user.phone}</div>
                         ...
                         <div className="text-right">
                           <div className="font-bold text-red-600">{days} 天未活動</div>
                           <div className="text-xs text-gray-400">最後活動: {formatDate(lastActive.toISOString())}</div>
                         </div>
                         <button onClick={() => setEditingUser(user)} className="p-2 text-gray-400 hover:text-indigo-600">
                           <Settings size={18} />
                         </button>
          ...
          <div className="space-y-6">
            {!selectedMonth ? (
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {submissionsByMonth.length === 0 && <div className="col-span-full text-center text-gray-500 py-10">目前尚無任何繳交紀錄</div>}
                {submissionsByMonth.map(([month, subs]) => {
                  const pendingCount = subs.filter(s => s.status === 'pending').length;
                  return (
                    <button 
                      key={month}
                      onClick={() => { setSelectedMonth(month); setFilterStatus('all'); setFilterTag('all'); }}
                    ...
                    >
                      ...
                      <Folder className="text-yellow-400 w-12 h-12 group-hover:scale-110 transition-transform" fill="currentColor" />
                      <span className="font-bold text-lg text-gray-700">{month}</span>
                      <div className="text-xs text-gray-500 mt-1">
                        {pendingCount > 0 ? <span className="text-red-500 font-bold">{pendingCount} 篇待批</span> : '全數已改'} / {subs.length} 總數
                      </div>
                    </button>
                  );
                })}
              ...
              <div className="bg-white rounded-lg shadow min-h-[500px]">
                ...
                <div className="p-4 border-b flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    <button onClick={() => setSelectedMonth(null)} className="text-indigo-600 hover:text-indigo-800 font-medium">
                        ← 返回資料夾
                    </button>
                    <h2 className="text-xl font-bold">{selectedMonth} 繳交紀錄</h2>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                    ...
                    <div className="relative">
                        ...
                        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700"><Filter size={12} /></div>
                    </div>
                    <div className="flex bg-gray-100 p-1 rounded-lg">
                      <button onClick={() => setFilterStatus('all')} className={`px-3 py-1 rounded text-xs font-medium transition ${filterStatus === 'all' ? 'bg-white shadow text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>全部</button>
                      <button onClick={() => setFilterStatus('pending')} className={`px-3 py-1 rounded text-xs font-medium transition ${filterStatus === 'pending' ? 'bg-white shadow text-red-600' : 'text-gray-500 hover:text-gray-700'}`}>待批改</button>
                      <button onClick={() => setFilterStatus('graded')} className={`px-3 py-1 rounded text-xs font-medium transition ${filterStatus === 'graded' ? 'bg-white shadow text-green-600' : 'text-gray-500 hover:text-gray-700'}`}>已批改</button>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-indigo-50 border-b border-indigo-100 p-4 grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in-down">
                      <div className="flex flex-col gap-4">
                        ...
                        <div className="flex items-center justify-around">
                          <div className="text-center"><p className="text-xs text-indigo-600 font-medium uppercase">平均分數</p><p className="text-3xl font-bold text-indigo-900">{monthAnalytics.avg}</p></div>
                          <div className="text-center"><p className="text-xs text-indigo-600 font-medium uppercase">最高分</p><p className="text-3xl font-bold text-indigo-900">{monthAnalytics.max}</p></div>
                          <div className="text-center"><p className="text-xs text-indigo-600 font-medium uppercase">批改進度</p><p className="text-3xl font-bold text-indigo-900">{monthAnalytics.total}<span className="text-sm font-normal text-indigo-500"> 篇</span></p></div>
                        </div>
                        {/* 主題分析 */}
                        <div className="bg-white p-3 rounded-lg shadow-sm border border-indigo-100">
                           <h4 className="text-xs font-bold text-indigo-800 mb-2 flex items-center gap-1"><BookOpen size={12}/> 主題成效分析</h4>
                           <div className="grid grid-cols-2 gap-2">
                             {Object.entries(monthAnalytics.topicStats).map(([topic, stats]) => (
                               <div key={topic} className="flex justify-between items-center text-xs p-1.5 bg-gray-50 rounded">
                                 <span className="text-gray-600 font-medium">{topic}</span>
                                 <span className="text-indigo-600 font-bold">{stats.avg} 分 <span className="text-gray-400 font-normal">({stats.count})</span></span>
                               </div>
                             ))}
                           </div>
                        </div>
                    ...
                    <div>
                       {/* 分數分佈 */}
                       <div className="flex justify-between items-center mb-1 text-xs text-indigo-700 font-medium"><span>成績分布</span><BarChart3 size={14} /></div>
                       <div className="space-y-1.5">
                        {/* A+ */}
                        <div className="flex items-center gap-2 text-xs">
                          <span className="w-12 text-right text-gray-600">A+級</span>
                          <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                            <div className="bg-emerald-600 h-1.5 rounded-full" style={{ width: `${(monthAnalytics.dist.levelAp / monthAnalytics.total) * 100}%` }}></div>
                          </div>
                          <span className="w-6 text-right text-gray-500">{monthAnalytics.dist.levelAp}</span>
                        </div>
                        {/* A */}
                        <div className="flex items-center gap-2 text-xs">
                          <span className="w-12 text-right text-gray-600">A級</span>
                          <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                            <div className="bg-emerald-400 h-1.5 rounded-full" style={{ width: `${(monthAnalytics.dist.levelA / monthAnalytics.total) * 100}%` }}></div>
                          </div>
                          <span className="w-6 text-right text-gray-500">{monthAnalytics.dist.levelA}</span>
                        </div>
                        {/* B+ */}
                        <div className="flex items-center gap-2 text-xs">
                          <span className="w-12 text-right text-gray-600">B+級</span>
                          <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                            <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${(monthAnalytics.dist.levelBp / monthAnalytics.total) * 100}%` }}></div>
                          </div>
                          <span className="w-6 text-right text-gray-500">{monthAnalytics.dist.levelBp}</span>
                        </div>
                        {/* B */}
                        <div className="flex items-center gap-2 text-xs">
                          <span className="w-12 text-right text-gray-600">B級</span>
                          <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                            <div className="bg-yellow-500 h-1.5 rounded-full" style={{ width: `${(monthAnalytics.dist.levelB / monthAnalytics.total) * 100}%` }}></div>
                          </div>
                          <span className="w-6 text-right text-gray-500">{monthAnalytics.dist.levelB}</span>
                        </div>
                        {/* C+ */}
                        <div className="flex items-center gap-2 text-xs">
                          <span className="w-12 text-right text-gray-600">C+級</span>
                          <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                            <div className="bg-orange-500 h-1.5 rounded-full" style={{ width: `${(monthAnalytics.dist.levelCp / monthAnalytics.total) * 100}%` }}></div>
                          </div>
                          <span className="w-6 text-right text-gray-500">{monthAnalytics.dist.levelCp}</span>
                        </div>
                        {/* C */}
                        <div className="flex items-center gap-2 text-xs">
                          <span className="w-12 text-right text-gray-600">C級</span>
                          <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                            <div className="bg-red-500 h-1.5 rounded-full" style={{ width: `${(monthAnalytics.dist.levelC / monthAnalytics.total) * 100}%` }}></div>
                          </div>
                          <span className="w-6 text-right text-gray-500">{monthAnalytics.dist.levelC}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="overflow-x-auto">
                   <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">繳交日期</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">學員姓名</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {currentViewSubmissions.map(sub => (
                        <tr key={sub.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDateTime(sub.date)}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{sub.studentName}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${sub.status === 'graded' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                              {sub.status === 'graded' ? '已批改' : '待批改'}
                            </span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{sub.score !== null ? sub.score : '-'}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <button onClick={() => openGradingModal(sub)} className="text-indigo-600 hover:text-indigo-900 flex items-center gap-1">
                              <FileText size={16} /> {sub.status === 'graded' ? '修改' : '批改'}
                            </button>
                            {/* 顯示主題標籤 */}
                            {sub.topic && <span className="ml-2 text-xs bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">{sub.topic}</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
        ...
        {activeTab === 'students' && (
          <div className="bg-white shadow rounded-lg overflow-hidden">
             <div className="p-4 bg-gray-50 border-b flex justify-between items-center flex-wrap gap-4">
                <div className="flex items-center gap-4">
                  <h3 className="font-bold text-gray-700">學員列表 ({users.length}人)</h3>
                  {/* 批量操作工具列 */}
                  {selectedUserIds.size > 0 && (
                    <div className="flex items-center gap-2 bg-red-50 px-3 py-1.5 rounded-lg border border-red-200 animate-fade-in">
                        <span className="text-sm font-bold text-red-700">已選取 {selectedUserIds.size} 人</span>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">姓名/手機</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">標籤</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:text-indigo-600" onClick={() => handleSort('remainingDays')}>
                      <div className="flex items-center gap-1">
                        剩餘天數 
                        {sortConfig.key === 'remainingDays' && (sortConfig.direction === 'asc' ? <ChevronUp size={14}/> : <ChevronDown size={14}/>)}
                      </div>
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">已交篇數</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">權限狀態</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">設定</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {sortedUsers.map(u => {
                    ...
                    if (days < 7) {
                        ...
                        warningIcon = <AlertTriangle size={16} className="text-red-600 inline mr-1" />;
                    } else if (days <= 30) {
                        dayColorClass = "text-yellow-600 font-bold";
                    }

                    return (
                      <tr key={u.id} className={isSelected ? "bg-indigo-50" : ""}>
                        <td className="px-6 py-4 whitespace-nowrap">
                            <input 
                                type="checkbox" 
                                className="rounded text-indigo-600 focus:ring-indigo-500 cursor-pointer w-4 h-4"
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex flex-wrap gap-1 max-w-[150px]">
                                {u.tags && u.tags.map((tag, idx) => (
                                    <span key={idx} className="bg-gray-100 text-gray-600 text-[10px] px-1.5 py-0.5 rounded border border-gray-200">#{tag}</span>
                                ))}
                            </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            {warningIcon}
                            <span className={`text-sm ${dayColorClass}`}>{days} 天</span>
                          </div>
                          <div className="text-xs text-gray-400">{formatDate(u.expiryDate)} 到期</div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {u.isManualLocked ? (
                            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">已停權</span>
                          ) : (
        ...
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up">
            <div className="bg-yellow-50 p-4 border-b border-yellow-100 flex justify-between items-center">
              <h3 className="font-bold text-lg text-yellow-800 flex items-center gap-2"><Megaphone size={20} /> 班級公告管理</h3>
              <button onClick={() => setIsAnnouncementModalOpen(false)} className="text-gray-400 hover:text-gray-600"><XCircle size={24} /></button>
            </div>
            <div className="p-6">
              <div className="mb-4">
                  <label className="block text-sm font-bold text-gray-700 mb-2">發送對象 (不選則為全體)</label>
                  <div className="flex flex-wrap gap-2">
                      {availableTags.map(tag => (
                          <button key={tag} onClick={() => toggleAnnouncementTag(tag)} className={`text-xs px-2 py-1 rounded border transition ${announcementTargetTags.includes(tag) ? 'bg-yellow-500 text-white border-yellow-600' : 'bg-gray-100 text-gray-600 border-gray-200'}`}>#{tag}</button>
                      ))}
                  </div>
              </div>
              <textarea rows="6" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 outline-none resize-none mb-4" placeholder="請輸入公告內容..." value={announcementText} onChange={(e) => setAnnouncementText(e.target.value)}/>
              <div className="flex gap-3">
                <button onClick={() => handleSaveAnnouncement(false)} className="flex-1 py-2 border border-gray-300 text-gray-600 rounded-lg hover:bg-gray-50 font-medium">隱藏公告</button>
                <button onClick={() => handleSaveAnnouncement(true)} className="flex-1 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 font-bold shadow">發布公告</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh] animate-fade-in-up">
            <div className="bg-gray-50 p-4 border-b flex justify-between items-center flex-shrink-0">
              ...
              <button onClick={() => setEditingUser(null)} className="text-gray-400 hover:text-gray-600"><XCircle size={24} /></button>
            </div>
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              <div className="space-y-4 border-b border-gray-100 pb-4 mb-4">
                  <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                         <label className="block text-xs text-gray-500 mb-1">學員姓名</label>
                         <input 
                           type="text" 
                           value={editName} 
                  ...
                  <div className="pl-[60px]">
                      <label className="block text-xs text-gray-500 mb-1">手機號碼 (登入帳號)</label>
                      <input 
                          type="tel" 
                          value={editPhone} 
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-3">
                 <label className="block text-sm font-bold text-gray-700 mb-2">學員標籤</label>
                 <div className="flex flex-wrap gap-2 mb-3">
                    {editingUser.tags && editingUser.tags.map((tag, idx) => (
                        <span key={idx} className="bg-indigo-50 text-indigo-700 text-xs px-2 py-1 rounded flex items-center gap-1">#{tag}<button onClick={() => removeTagFromUser(editingUser, tag)} className="hover:text-red-500"><X size={12}/></button></span>
                        ))}
                     </div>
                     <div className="flex gap-2">
                        <input type="text" placeholder="新增標籤" className="flex-1 px-3 py-1.5 border border-gray-300 rounded text-sm outline-none focus:border-indigo-500" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter') addTagToUser(editingUser, tagInput) }}/>
                        <button onClick={() => addTagToUser(editingUser, tagInput)} className="bg-indigo-600 text-white px-3 py-1.5 rounded text-sm font-bold hover:bg-indigo-700"><Plus size={16} /></button>
                     </div>
                 </div>
              ...
              <div className="space-y-4">
                <div className="bg-gray-50 p-4 rounded-lg flex justify-between items-center">
                  <div>
                    <span className="block font-medium text-gray-700">繳交權限</span>
                    <span className="text-xs text-gray-500">{editingUser.isManualLocked ? '目前：已鎖定' : '目前：正常可繳交'}</span>
                  </div>
                  <button 
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  <span className="block font-medium text-gray-700 mb-2">課程延期 (目前: {formatDate(editingUser.expiryDate)})</span>
                  <div className="grid grid-cols-3 gap-2">
                    <button onClick={() => extendExpiry(editingUser, 1)} className="bg-white border hover:bg-blue-50 hover:border-blue-300 text-blue-600 py-2 rounded text-sm font-medium transition">
                      + 1 個月
                    </button>
                    <button onClick={() => extendExpiry(editingUser, 2)} className="bg-white border hover:bg-blue-50 hover:border-blue-300 text-blue-600 py-2 rounded text-sm font-medium transition">
                      + 2 個月
                    </button>
                    <button onClick={() => extendExpiry(editingUser, 3)} className="bg-white border hover:bg-blue-50 hover:border-blue-300 text-blue-600 py-2 rounded text-sm font-medium transition">
                      + 3 個月
                    </button>
                  </div>
                </div>
                ...
                <button 
                  ...
                  className={`w-full flex items-center justify-center gap-2 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white py-3 rounded-lg font-bold shadow-md transition transform hover:scale-[1.02] ${isExporting ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                ...
                <button 
                  ...
                  disabled={isExporting}
                  className={`w-full flex items-center justify-center gap-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 py-3 rounded-lg font-medium transition ${isExporting ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {isExporting ? <Loader className="animate-spin" size={18} /> : <Mail size={18} />} 
